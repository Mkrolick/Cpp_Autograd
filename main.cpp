...
int main(int argc, char* argv[]) {
    ...
    // Update input vector to match network input size (4)
    std::vector<std::vector<double>> x = {
        {1,2,3,4},
        {-1,-2,-3,-4},
        {1,3,4,5},
    ...
    MLP perceptron = MLP(4, {3, 2, 1});


    // Initialize a single neuron with 4 inputs
    Neuron n = Neuron(1);
    
    // Test the neuron with sample input
    Value a = Value(1.0);

    std::vector<Value> test_input = {a};
    Value output = n.call(test_input);

    output.full_backwards();
    


    std::cout << **(output.children.begin()) << std::endl;

    (*output.children.begin())->data = 4;

    std::cout << **(output.children.begin()) << std::endl;

    std::cout << *n.parameters().at(1) << std::endl;

    std::cout << *n.parameters().at(0) << std::endl;
    */
